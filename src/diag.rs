...
pub struct Diag {
    ...
    /// when the record is built, package-relative. Boxed so the field is one
    /// thin pointer: `Diag` is the `Err` of every parser function, and an
    /// inline `String` (or even a `Box<str>`) puts it over clippy's
