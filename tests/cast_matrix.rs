...
fn unsupported(body: &str, names: &str) {
    let observation = observe(body);
    assert_eq!(
        observation.verdict,
        Verdict::Unsupported,
...
/// The filed program, byte for byte: `to_f(3)` is `3.0` and prints as one.
#[test]
...
";
    ...
    assert_eq!(String::from_utf8_lossy(&observation.stdout).trim(), "3.0");
}
...
fn bool_does_not_cast_to_numbers() {
    unsupported("let b = true\nlet n = b as int\nn", "truthiness");
    unsupported("let b = false\nlet f = b as f64\n0", "truthiness");
}

#[test]
fn numbers_do_not_cast_to_str() {
    unsupported("let s = 3 as str\n0", "E0805");
    unsupported("let s = 3.5 as str\n0", "E0805");
}
