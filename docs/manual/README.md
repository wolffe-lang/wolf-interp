# The wolf-interp manual

How to build the reference interpreter, run wolf programs on it, and read
what its tools print. One sentence of ground rules: the
...
1. [Running programs](01-running-programs.md) — `conform-run`, exit codes,
   traps and what they mean.
2. [The REPL](02-repl.md) — a session walkthrough; `:type`, `:regions`,
   `:trace`, `:schedule`; multi-line input.
...
One line per command, kept identical to `wolf-interp --help`
