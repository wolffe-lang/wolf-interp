...
lupin: 242 entries reach the `run` rung; 223 match their `check:` expectation, 13 are the dynamic counterpart of the static code the corpus pins, 33 are static-conservatism entries (the compiler rejects statically what this machine never checks), 48 are out of scope, 0 mismatch
```
