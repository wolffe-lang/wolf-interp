...
lupin: 203 entries reach the `run` rung; 188 match their `check:` expectation, 8 are the dynamic counterpart of the static code the corpus pins, 34 are static-conservatism entries (the compiler rejects statically what this machine never checks), 50 are out of scope, 0 mismatch
```
