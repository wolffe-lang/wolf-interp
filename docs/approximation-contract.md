...
(`[conf.trap.set]`). The rule, and through it the clause anchor,
...
Detection is a comparison: a dangling reference is a live index whose region
generation or slot generation no longer matches.
...
`[mem.region.edge.iso]` lets one region own another. An owner's open window
reaches its child's data, the aliasing Verona's single-window rule exists to
forbid. Two distinct affine region values in an
...
corpus is not the defendant, the reading that survives is that re-entering a
region which is already open is idempotent (a depth count, not a second
...
2. The disjointness hole in §5.1 is *reachable only* through an iso field,
   which makes the antichain repair cheap.
...
The corpus is not the defendant, so nothing was adjusted here.
...
`shared` cells. Use-after-free is exact over that set.
...
the freed-home fault (#25) still fires on any later access, so
`region_escape_local.lu`'s family traps as before.
...
still disagree with each other, which is a wart. Filed
...
lowercase bindings), and the failure mode is an `unsupported` ("no `match`
arm applied"); it never answers wrongly. Exhaustiveness stays the type
...
E1013 (not the old accidental E1001-reads-as-moves story), and
...
- `[mem.iter.for]`. `for` over a non-builtin operand looks up `next`
  ...
  as the bare `else` reads. Method dispatch is dynamic (by the
...
On every defined shape the two machines agree, which is the
...
Consequence: no conforming program can observe this, because the
compiler rejects the shape statically and the corpus pins that
rejection. It is not a divergence and it does not gate. It is a place
where this machine would fail to be an oracle if the compiler's static
check were ever wrong, which is the situation the differential exists to
catch, so it is recorded here.
...
value's `home` against its region's liveness. `home` today has one
reader, `Machine::frozen_container`, and it asks one question
...
That makes the fix a bounded, three-part change, written down here so a
later sprint can execute it without re-deriving it:
...
1. Give `List` and `Map` a home. `Value::List(Vec<Slot>,
   ...
   matched in scores of places), and it is why this needs a sprint.
...
3. Keep identity out of both equalities. The language's
...
The re-differential costs more than the code: this adds new dynamic
faults, so every corpus entry that builds a container inside a region has
to be re-compared on all three counterparty tiers, and any program that
legitimately reads a container after its region closes (there should be
none) becomes a finding to triage.

Decision, this pass: NOT DONE, a named design task. It is a change to
§6.1's granules-with-identity rule, it changes the trap surface, and this
pass already moved the pin and rewrote the method-call path for
wolf-interp#24. Landing all three together would make any resulting
divergence un-bisectable.
...
1. It is a high-water accounting, not an occupancy one. It answers "how
   much storage has this region been asked for", which is the monotone
...
2. `str` charges here, and does not charge on the counterparty's native
   ...
   charges". The two witnesses print relation *booleans* so the three lanes
...
`[mem.region.freeze.1]` means it is never freed, which is §4's leak reading
...
booleans, as it is for the account witnesses.

One consequence specific to this tier: §6.15's `str` divergence
(wolf-lang#191: this machine charges a `str` to the ambient region where
native charges the process root) is now *visible as a refusal*, not only as a
number. A capped region here can breach on string bytes that would
...
highest-severity class the protocol has.
...
call-by-value-result writes the result back) and `read` does not. The Frozen child
...
1. `a` and `a.x` are unrelated in the provenance forest. Their place keys
   ...
   is02's `AccessSet`) already decides that case, by the same
...
true of this machine by construction, and
...
platform-independent, and not a real address.
...
exclusivity. is03's machine enforces both and produces traps, not UB.
...
pass confirmed it (issue #19): the clause is `[mem.unsafe.raw.2]`.
...
Flagged so that a later alignment with the paper is an amendment and not a
silent divergence.
...
`[conc.mm.race.1]`'s reachability. A vector-clock race detector
...
uses (frame-serial + name + write generation), and a call on the
...
pins. Task closures stay under D14's copy law and E1101 above; their loans
are exempt by task identity.
...
- Scope join under cancellation keeps joining. *Confirmed and
  ...
  joining, as before. The owner's surfaced cancellation error is
...
- Deadlock. *Deviation paid as specified*: `unsupported`-with-roster
  ...
  outcome, `[conc.deadlock.trap]` adds `deadlock` as the
...
- The branch alphabet is two decision kinds. Every schedule point of
  is06's enumeration funnels into `State::decide` at two places:
...
- Conflicts are syntactic over runtime objects. Two ops conflict when
  ...
  costs exploration and never soundness. One refinement matters:
...
- The packed-seed namespace is provisional (finding S-9). Bit 62 of a
  ...
  generator as before, so all pre-is07 seeds and snapshots are untouched. Streams too wide for 62 bits use `--schedule=ev:c0,c1,…`.
...
There was one corpus file where the difference was visible:
...
The check is partial, and this is the declared part. It
