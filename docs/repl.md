...
- Line-oriented. The prompt is `wolf> `; a line that cannot terminate yet
  ...
  separator as spec/01 has it (E0002 owns the empty statement).
...
- Every diagnostic renders with its code and clause anchor
  ...
  Spans index the input line as typed, as byte offsets, where the file
  doors say `line:col` (`[conf.trap.render]`): a session has no stable
  line numbering (each entry restarts at offset 0, and an ownership
  fault's second span may point into an *earlier* entry), so the
  coordinate is entry-relative. The Elm-grade catalog
...
outside spec/01..05: the REPL extends the spec without forking it. is09
exports them with the rest of the doc surface.

- `[repl.def.shadow]`. Redefinition shadows. A new `fn f` binds fresh;
  closures and values that captured the old `f` keep it. There is no
  live-patching of existing values; that is I14's compiled-world story. (Mechanism: every prompt definition gets a
...
- `[repl.trap.alive]`. A trap, UB finding, or diagnostic prints and
  ...
  no rollback, and `:mem` shows that state; it is a teaching surface.
  `:reset` is the fresh start. The survival holds on every fault; the
  reminder line (`the session survives …`) prints once per session (on the
  first fault, trap or UB), so a transcript with a run of faults does not
  repeat it.
...
A transcript is what a piped session prints: prompts, echoed
...
region lifecycle as a book chapter would stage it.
...
The prompt line-edits. These notes are the editing contract (is08's
discipline, applied to the reader). The editor is a *reader*: it hands
complete inputs to `Session::feed_line` and owns no meaning; every semantic
in sections 1–5 is untouched by it.

- `[repl.edit.tty]`. The editor engages only when stdin is a terminal.
  A piped session keeps the dumb reader, prompts (`wolf> `/`....> `),
  ...
  degrades to the dumb reader with a one-line stderr note, and a prompt
  still opens. Under the editor, a continuation is one editable buffer
  rendered without the `....> ` marker; `....> ` remains transcript syntax,
  printed by the piped path as before.
- `[repl.edit.dep]`. The editor is `rustyline` 18 (MIT; MIT composes
  with this repository's GPL-3.0-or-later, and the combined work remains
  GPL-3.0-or-later). The dependency is required: `unsafe_code = "forbid"`
  stands, so raw-mode termios cannot be hand-rolled here. Rejected
  alternative: `reedline` (more modern, nushell-shaped keybinding
  vocabulary, weaker GNU-readline correspondence; the ask was readline
  fidelity, and rustyline's `Validator` hook maps onto
...
- `[repl.edit.keys]`. The GNU-readline vocabulary: motion
  ...
  freebies raw mode removes) are rebound. No binding is claimed from
  crate documentation: every one is verified by keystroke
...
- `[repl.edit.history]`. Up/Down recall in-session and across
  ...
  recorded. A multi-line input is one history item, so recall brings back
  the whole `fn`, and the file format is one JSON string per line so
  embedded newlines survive the round trip. A corrupt line is skipped.
  `Ctrl-R` searches the same list.
...
- `[repl.edit.complete]`. TAB completes from three sources the process
  ...
  session's own bound names, surface names only, with the generational
  internals (`f#2`, `Point#1`) of `[repl.def.shadow]`/`[repl.type.gen]`
  kept out; and filesystem paths after `:load`. Ambiguity lists the
  candidates.
...
behind its config and it is not exposed; residue, one config line if
